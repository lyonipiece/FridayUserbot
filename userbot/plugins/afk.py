    ...
    if ".afk" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        try:
            await borg.send_message(  # pylint:disable=E0602
                ...
                "My Owner he is very busy now. He has some urgent works 😅"
            )
...
async def _(event):
    ...
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            await event.edit(f"My Boss leaves 😗, because of {reason}")
        else:
            await event.edit(f"My Boss leaves")
        await asyncio.sleep(5)
        await event.delete()
        try:
            await borg.send_message(  # pylint:disable=E0602
                Config.PLUGIN_CHANNEL,  # pylint:disable=E0602
                f"My Boss leaves, because of {reason}"
            )
        except Exception as e:  # pylint:disable=C0103,W0703
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = f"My Master has leave for {afk_since}\nWhere He Is: Tereko kyu batau " + \
